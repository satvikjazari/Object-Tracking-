...
#sys.path.append('../')
#print("The path till here is", sys.path)
from darkflowTest.net.build import TFNet
...
# this function is basically returning all the objects with their bounding box and confidence
# here we will replace this with the way that is happening in our approach where we just get a frame
# def postprocess(frame, out, threshold, classes, framework):
def postprocess(frame, threshold):
    ...
    #             #print(label_with_num + ' at co-ordinates '+ str(objects_detected[label_with_num]))
        ...
        # here we will not use this chunk of code
    ...
    for face in faces:
        ...
        if confidence > threshold:
            i = 0
            while(True):
                if i not in objects_detected.keys():
                    break
                i=i+1
            width = face.bounding_box[1]-face.bounding_box[0]
            height = face.bounding_box[2]-face.bounding_box[3]
            objects_detected[i] = [(int(face.bounding_box[0]),int(face.bounding_box[2]),int(width),int(height)),confidence]
    return objects_detected



def intermediate_detections(stream, threshold):
    _, frame = stream.read()

    # this is the line where there are predictions of the network
    # here predictor is the object detector which returns a blob of predictions
    # predictions = predictor.predict(frame)    #we do not need this now
    # here it is detecting the objects by calling function
    # objects_detected = postprocess(frame, threshold, predictor.framework)
    objects_detected = postprocess(frame, threshold)

    objects_list = list(objects_detected.keys())
    print('Tracking the following objects', objects_list)

    # now we associate trackers with each of the detected objects
    trackers_dict = dict()
...
def process(args):
    ...
    # here the object detector is called
    ...
    stream, objects_detected, objects_list, trackers_dict = intermediate_detections(stream,  args.thr)
